 ...
 *
 */
public class PostgresDBShutdownHookThread extends Thread {

	// 로그
	private static final Logger logger = LogManager.getLogger(PostgresDBShutdownHookThread.class);
	...
	public void run() {
			...
			try {
				prcsDAO.updateProcsStatus(ConstDef.PROCS_STTUS_CD.OFF, "PostgresDB");
			} catch (Exception e) {
				System.out.println("Exception Message updateProcsStatus" );
			}

			//logger.info("PostgresDB Shutdown(OFF:002)");
